...
pub fn route53_hosted_zone_config() -> AwsccSchemaConfig {
    AwsccSchemaConfig {
        ...
        .attribute(
            AttributeSchema::new("vp_cs", AttributeType::unordered_list(AttributeType::struct_("VPC".to_string(), vec![StructField::new("vpc_id", carina_aws_types::vpc_id()).required().with_description("*Private hosted zones only:* The ID of an Amazon VPC. For public hosted zones, omit ``VPCs``, ``VPCId``, and ``VPCRegion``.").with_provider_name("VPCId"),
                    StructField::new("vpc_region", carina_aws_types::aws_region()).required().with_description("*Private hosted zones only:* The region that an Amazon VPC was created in. For public hosted zones, omit ``VPCs``, ``VPCId``, and ``VPCRegion``.").with_provider_name("VPCRegion")])))
