//! hosted_zone schema definition for AWS Cloud Control
//!
//! Auto-generated from CloudFormation schema: AWS::Route53::HostedZone
//!
...
pub fn route53_hosted_zone_config() -> AwsccSchemaConfig {
    AwsccSchemaConfig {
        ...
        .attribute(
            AttributeSchema::new("hosted_zone_features", AttributeType::struct_("HostedZoneFeatures".to_string(), vec![StructField::new("enable_accelerated_recovery", AttributeType::bool()).with_description("").with_provider_name("EnableAcceleratedRecovery")]))
        ...
        .attribute(
            AttributeSchema::new("name_servers", AttributeType::list(AttributeType::string()))
                .read_only()
                .with_description(" (read-only)")
                .with_provider_name("NameServers"),
        )
        .attribute(
            AttributeSchema::new("query_logging_config", AttributeType::struct_("QueryLoggingConfig".to_string(), vec![StructField::new("cloud_watch_logs_log_group_arn", super::arn()).required().with_description("The Amazon Resource Name (ARN) of the CloudWatch Logs log group that Amazon Route 53 is publishing logs to.").with_provider_name("CloudWatchLogsLogGroupArn")]))
        ...
        .attribute(
            AttributeSchema::new("vp_cs", AttributeType::unordered_list(AttributeType::struct_("VPC".to_string(), vec![StructField::new("vpc_id", super::vpc_id()).required().with_description("*Private hosted zones only:* The ID of an Amazon VPC. For public hosted zones, omit ``VPCs``, ``VPCId``, and ``VPCRegion``.").with_provider_name("VPCId"),
                    StructField::new("vpc_region", super::awscc_region()).required().with_description("*Private hosted zones only:* The region that an Amazon VPC was created in. For public hosted zones, omit ``VPCs``, ``VPCId``, and ``VPCRegion``.").with_provider_name("VPCRegion")])))
                .with_description("*Private hosted zones:* A complex type that contains information about the VPCs that are associated with the specified hosted zone. For public hosted zones, omit ``VPCs``, ``VPCId``, and ``VPCRegion``.")
                .with_provider_name("VPCs")
                .with_block_name("vp_c"),
        )
