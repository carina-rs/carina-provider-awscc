...
pub fn route53_hosted_zone_config() -> AwsccSchemaConfig {
    AwsccSchemaConfig {
        ...
        .attribute(
            AttributeSchema::new("hosted_zone_tags", AttributeType::unordered_list(AttributeType::Struct {
        ...
        .attribute(
            AttributeSchema::new("vp_cs", AttributeType::unordered_list(AttributeType::Struct {
                    name: "VPC".to_string(),
                    fields: vec![
                    StructField::new("vpc_id", super::vpc_id()).required().with_description("*Private hosted zones only:* The ID of an Amazon VPC. For public hosted zones, omit ``VPCs``, ``VPCId``, and ``VPCRegion``.").with_provider_name("VPCId"),
